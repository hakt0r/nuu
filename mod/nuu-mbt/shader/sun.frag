...
float shading(vec4 position){
  vec3 l = normalize(light);
  return max(0.08, dot(vNormal, l)); }

vec3 fineNoise(vec2 p){
...
void main() {
  vec2 p = vTex;
	...
	float unGamma = 1.0 / 0.7;
	// base + noise
	color.rgb = temperature.rgb + temperature.rgb * plasma(p * 2000.) * fineNoise(p);
	// float theta = 1.0 - dot(0.5, p);
	// color = color + total - theta;
